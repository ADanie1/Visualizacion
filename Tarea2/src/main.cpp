...
// Caso base:
        if (n == 1) {
        // Se generan los tres colores aleatorios (usando nombres distintos para no chocar con a, b y c)
        float uno = static_cast<float>(std::rand()) / RAND_MAX;
        float dos = static_cast<float>(std::rand()) / RAND_MAX;
        float tres = static_cast<float>(std::rand()) / RAND_MAX;

        // Se guarda el primer vértice (x, y, z=0.0) y sus colores
        vertices.push_back(a.x); vertices.push_back(a.y); vertices.push_back(0.0f);
        vertices.push_back(uno);  vertices.push_back(dos);  vertices.push_back(tres);

        // Se guarda el segundo vértice
        vertices.push_back(b.x); vertices.push_back(b.y); vertices.push_back(0.0f);
        vertices.push_back(uno);  vertices.push_back(dos);  vertices.push_back(tres);

        // Se guarda el tercer vértice
        vertices.push_back(c.x); vertices.push_back(c.y); vertices.push_back(0.0f);
        vertices.push_back(uno);  vertices.push_back(dos);  vertices.push_back(tres);
...
int main()
{
    ...
    // Configuración para las posiciones (location = 0 en el Vertex Shader)
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)0);
    glEnableVertexAttribArray(0);

    // Configuración para los colores (location = 1 en el Vertex Shader)
    // El offset inicia después de los primeros 3 valores de posición
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Draw the triangle (dentro del while loop)
